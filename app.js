...
// Brand marks from Simple Icons (CC0). Single 24x24 paths, filled with
// currentColor so they inherit the page's monochrome palette in both themes.
var LOGOS = {
...
// Agents this runs in. `logo` is a LOGOS key; agents with no CC0 mark available
// fall back to their name set in the mono face.
var SUPPORTED = [
...
(function () {
  ...
  if (row) {
    SUPPORTED.forEach(function (a) {
      ...
      item.className = 'logo' + (a.logo ? '' : ' logo-text');
      item.title = a.name;
      item.innerHTML = a.logo
        ? '<svg viewBox="0 0 24 24" role="img" aria-label="' + escapeHtml(a.name) + '">' +
          '<path fill="currentColor" d="' + LOGOS[a.logo] + '"/></svg>'
        : escapeHtml(a.name);
